...
// Skins are recolors applied via a canvas filter at draw time, so no new
// sprite art is needed. Unlocked purely by comparing against the best
// score already saved in localStorage - no separate unlock list required.
const BIRD_SKINS = [
  { id: "classic", name: "Classic", unlockScore: 0, filter: "none", swatch: "#f5d90a" },
  { id: "golden", name: "Golden", unlockScore: 10, filter: "hue-rotate(25deg) saturate(1.8) brightness(1.15)", swatch: "#ffd700" },
  { id: "ruby", name: "Ruby", unlockScore: 25, filter: "hue-rotate(-100deg) saturate(2.2)", swatch: "#e0115f" },
  { id: "shadow", name: "Shadow", unlockScore: 50, filter: "grayscale(1) brightness(0.45)", swatch: "#333333" },
  { id: "neon", name: "Neon", unlockScore: 100, filter: "hue-rotate(190deg) saturate(3) brightness(1.3)", swatch: "#39ff14" },
];
...
function renderSkinSelector() {
  ...
  BIRD_SKINS.forEach((skin) => {
    ...
    btn.className =
      ...
      (unlocked ? "" : " locked");
    btn.title = unlocked ? skin.name : `${skin.name} - unlock at ${skin.unlockScore} points`;
...
const bird = {
  ...
  draw: function () {
    ...
    sctx.filter = getCurrentSkin().filter;
