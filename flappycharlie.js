...
function previewSkin(id) {
  // Never let a locked skin be equipped mid-run - previewing is only
  // allowed while the bird is idle (getReady) or dead (gameOver).
  if (state.curr === state.Play) return;
  previewSkinId = id;
...
function getCurrentSkin() {
  // Preview only ever applies outside of active gameplay, so a locked
  // skin can never actually be "used" to play a run.
  if (previewSkinId && state.curr !== state.Play) {
